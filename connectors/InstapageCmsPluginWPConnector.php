...
class InstapageCmsPluginWPConnector {
  ...
  {
    return parse_url(wp_guess_url(), PHP_URL_HOST);
  }
  ...
  public function initPlugin() {
    InstapageCmsPluginHelper::writeDiagnostics(
        ...
        'Instapage plugin initiated. REQUEST_URI'
    );
  ...
  function checkPluginRequirementsOnActivation() {
    if (version_compare(PHP_VERSION, INSTAPAGE_PLUGIN_MINIMUM_PHP_VERSION, '<')) {
      deactivate_plugins(INSTAPAGE_PLUGIN_PATH . '/instapage.php');
        wp_die(
            sprintf(
                '<h2>Outdated PHP version</h2><p>%s%s%s%s%s</p>',
                __('Our Instapage plugin requires PHP '),
                INSTAPAGE_PLUGIN_MINIMUM_PHP_VERSION,
                __(' or newer. '),
                __('Your current PHP version is: '),
                PHP_VERSION
            ),
  ...
  public function checkProxy() {
    ...
    if ($services->isServicesRequest()) {
      try {
        ...
        echo InstapageCmsPluginConnector::escapeHTML($e->getMessage());
      }
    }
  }
