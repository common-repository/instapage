    ...
    public function display($page, $forcedStatus = null)
    {
        ...
        if (!empty($_GET)) {
            if ($lpAjaxLoaderController->shouldDecodeQuery()) {
                $querySufix = '?' . base64_decode(InstapageCmsPluginHelper::filterInput(INPUT_GET, 'b64'));
            } else {
                $querySufix = '?' . http_build_query(filter_input_array(INPUT_GET) ?: []);
            }
        } elseif (isset($_SERVER['QUERY_STRING']) && !empty($_SERVER['QUERY_STRING'])) {
            $querySufix = '?' . InstapageCmsPluginHelper::filterInput(INPUT_SERVER, 'QUERY_STRING');
        }
        ...
        if ($forcedStatus) {
            ...
            $status = isset($enterpriseCallResult['code']) ? (int) $enterpriseCallResult['code'] : 200;
        }
